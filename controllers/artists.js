...
router.get('/new/results', (req,res) => {
    var query = req.query;
    var url = "https://ws.audioscrobbler.com/2.0/?method=artist.search&artist="+query.name+"&limit=10&api_key="+"11d75a3fc31f58a0072eeaab6db42042"+"&format=json"
   console.log(url)
    axios.get(url)
    .then(response => {
...
router.get('/:id', isLoggedIn, (req,res) => {
    ...
    .then(artist => {
        ...
        var url = "https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist="+artist.name+"&api_key="+"11d75a3fc31f58a0072eeaab6db42042"+"&format=json"
        console.log(url)
        axios.get(url)
        .then(response => {
            ...
            if (onTour == 1) {
                let songkickURL = `https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist="+artist.name+"&api_key="+"11d75a3fc31f58a0072eeaab6db42042"+"&format=json`;
...
router.get('/:id/toptracks', (req,res) => {
    db.artist.findOne({where: {id: req.params.id}})
    .then(artist => {
        var url = "https://ws.audioscrobbler.com/2.0/?method=artist.gettoptracks&artist="+artist.name+"&limit=15&api_key="+"11d75a3fc31f58a0072eeaab6db42042"+"&format=json"
        axios.get(url)
        .then(response => {
            var results = response.data;
...
router.get('/:id/songsearch', (req,res) => {
    ...
    db.artist.findOne({where: {id: req.params.id}})
    .then(artist => {
        var url = "https://ws.audioscrobbler.com/2.0/?method=track.search&track="+query.name+"&artist="+ artist.name +"&limit=10&api_key="+"11d75a3fc31f58a0072eeaab6db42042"+"&format=json";
        axios.get(url)
        .then(response => {
            var results = response.data.results;
